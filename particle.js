window.settings = { sequenceColor: "#ffb6c1", matrixColor1: "#ffb6c1", matrixColor2: "#ffb6c1" };
function hexToRgb(hex) { var result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex); return result ? { r: parseInt(result[1], 16), g: parseInt(result[2], 16), b: parseInt(result[3], 16) } : { r: 255, g: 105, b: 180 }; }
...
S = {
    initialized: false,
    init: function () {
        // Removed landscape lock so it works on mobile portrait
...
S.UI = (function () {
    ...
    function performAction(value) {
        ...
        timedAction(function (index) {
            ...
            switch (action) {
                ...
                case 'memories':
                    if(typeof window.showMemories === 'function') {
                        window.showMemories();
                    }
                    break;

                case 'hold':
                    // Heart stays on screen. Notify script.js it's ready for click.
                    if(typeof window.onHeartReady === 'function') {
                        window.onHeartReady();
                    }
                    break;
    ...
    return {
        simulate: function (action) {
            // Removed landscape lock so it simulates on mobile portrait
    ...
    return {
        switchShape: function (n, fast) {
            ...
            while (n.dots.length > 0) {
                ...
                const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
                // Make the particles form words much smoother and softer on all devices
                dots[d].e = isMobile ? 0.12 : 0.08; 
            ...
            for (var i = d; i < dots.length; i++) {
                if (dots[i].s) {
                    ...
                    dots[i].s = false;
                    // Slow down the drifting background particles drastically
                    dots[i].e = 0.015; 
                    dots[i].move(new S.Point({
                        x: Math.random() * a.w,
                        y: Math.random() * a.h,
                        // Increased brightness to be visible but soft
                        a: 0.2, 
        ...
        explode: function() {
            ...
            for (var d = 0; d < dots.length; d++) {
                if (dots[d].s) {
                    dots[d].s = false;
                    // Make the explosion softer and slower
                    dots[d].e = 0.05; 
                    dots[d].move(new S.Point({
                        x: Math.random() * a.w * 2 - a.w * 0.5,
                        y: Math.random() * a.h * 2 - a.h * 0.5,
                        // Make explosion particles fade out fully
                        a: 0, 
                        z: Math.random() * 5,
                        h: 0
                    }));
